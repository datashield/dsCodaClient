# Creates server-side ILR coordinates for compositional data analysis
...
- V:
  ...
  an orthonormal basis matrix to be used for the ILR transform. If NULL
...
`ds.ilr` returns (assigns) a server-side object containing the
ILR-transformed coordinates for use in compositional data analysis.

## Details

This function creates a server-side ILR-transformed object that can be
used for compositional data analysis. It wraps the compositions::ilr()
function for use in DataSHIELD.

Server function called: `ilrDS`.
